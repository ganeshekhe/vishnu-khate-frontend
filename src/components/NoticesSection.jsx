...
const NoticesSection = () => {
  ...
  const getRandomIcon = (index) => {
    const Icon = noticeIcons[index % noticeIcons.length];
    return (
      <Icon className="text-yellow-400 text-xl flex-shrink-0 drop-shadow-md" />
    );
  };
  ...
  return (
    <div className="sticky top-0 z-50 w-full bg-gradient-to-r from-purple-900/95 via-indigo-900/95 to-purple-800/95 shadow-lg border-b border-purple-700">
      <div className="overflow-hidden whitespace-nowrap">
        <div className="inline-flex items-center animate-marquee">
          {notices.length === 0 ? (
            <span className="text-gray-300 px-4 py-2 md:px-6 md:py-3 italic tracking-wide">
              📰 No notices available.
            </span>
          ) : (
            [...notices, ...notices].map((notice, index) => (
              <div
                key={`${notice._id}-${index}`}
                className="flex items-center gap-3 px-4 py-2 md:px-6 md:py-3 hover:bg-purple-700/40 rounded-md transition-all duration-300"
              >
                {getRandomIcon(index)}
                {notice.url ? (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-teal-300 hover:text-teal-400 font-medium tracking-wide text-sm md:text-base transition-colors"
                  >
                    {notice.title}
                  </a>
                ) : (
                  <span className="text-gray-100 font-medium tracking-wide text-sm md:text-base">
                    {notice.title}
                  </span>
                )}
              </div>
