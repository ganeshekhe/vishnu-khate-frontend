...
const OperatorPanel = () => {
  ...
  useEffect(() => {
    ...
    socket.on("applicationDrafted", (draftApp) => {
      setApplications((prev) => {
        const exists = prev.find((a) => a._id === draftApp._id);
        if (exists) return prev.map((a) => (a._id === draftApp._id ? draftApp : a));
        return [draftApp, ...prev];
      });
    });
    ...
    socket.on("correctionSent", (updatedApp) => {
      setApplications((prev) => {
        const updatedApps = prev.map((a) => (a._id === updatedApp._id ? updatedApp : a));
        countStatus(updatedApps);
        return updatedApps;
      });
    });
  ...
  const handleNextStatus = async (appId, currentStatus) => {
    const nextStatus = nextStatusOf(currentStatus);
    if (!nextStatus) return toast.error("⚠️ No next status available");
    try {
      await axios.put(
        `${BASE_URL}/api/applications/${appId}/status`,
        { status: nextStatus },
      ...
      toast.success("➡️ Status updated to " + nextStatus);
      fetchApplications();
    } catch (err) {
      console.error(err);
      toast.error("❌ Failed to update status");
  ...
  const handleDownloadAllDocs = async (userId) => {
    try {
      ...
      alert(res.data.message || "Documents prepared for download");
    } catch (err) {
      console.error(err);
      alert("Failed to download all docs");
    }
  };

  const filteredApps = applications
    .filter((app) => (categoryFilter === "All" ? true : app.service?.category?.name === categoryFilter))
    .filter((app) => (statusFilter === "All" ? true : app.status === statusFilter))
    .filter((app) => app.user?.name.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    <div className="p-4">
      ...
      <table className="min-w-full border border-gray-300">
        <thead>
          <tr className="bg-gray-100">
            ...
            <th className="border p-2">Operator ID / Password</th>
            <th className="border p-2">Form PDF</th>
        ...
        <tbody>
          {filteredApps.map((app, idx) => (
            <tr key={app._id} className="text-center">
              ...
              <td className="border p-2">{app.service?.name}</td>
              <td className="border p-2">{app.status}</td>
              <td className="border p-2">
                <input
                  type="text"
                  placeholder="Operator ID"
                  value={selectedFiles[app._id]?.operatorId || app.operatorId || ""}
                  onChange={(e) => handleInputChange(e, app._id, "operatorId")}
                ...
                <input
                  type="text"
                  placeholder="Password"
                  value={selectedFiles[app._id]?.operatorPassword || app.operatorPassword || ""}
                  onChange={(e) => handleInputChange(e, app._id, "operatorPassword")}
              ...
              <td className="border p-2">
                ...
                {app.formPdf && (
                  <button
                    onClick={() => handleDownload(app.formPdf)}
                ...
                {app.status === "Waiting" && (
                  <>
                    ...
                    <button
                      ...
                      disabled={uploadingAppId === app._id}
                    >
                      {uploadingAppId === app._id ? "Uploading..." : "Upload Certificate"}
                    </button>
                  </>
                )}
                {app.certificate && (
                  <button
                    onClick={() => handleDownload(app.certificate)}
                    className="ml-2 text-green-600 underline"
                  >
      ...
      {showProfileModal && selectedProfile && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
          <div className="bg-white p-4 rounded w-3/4 max-h-[90vh] overflow-auto relative">
            <button
              onClick={() => setShowProfileModal(false)}
            ...
            <h2 className="text-xl font-bold mb-2">{selectedProfile.name} - Profile</h2>
            ...
            <div className="mt-4">
              ...
              {Object.keys(docLabels).map((key) => {
                const docs = selectedProfile[key] || [];
                return (
                  <div key={key} className="mt-2">
                    ...
                    {docs.map((file, idx) => (
                      <div key={idx} className="flex gap-2 items-center">
                        ...
                        <button
                          onClick={() => handleDeleteDoc(key, idx)}
                          className="text-red-600 px-1 rounded border"
                        >
                          Delete
                        </button>
                      </div>
