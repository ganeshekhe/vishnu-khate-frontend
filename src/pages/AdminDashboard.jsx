...
function AdminPanel() {
  ...
  const [newSubService, setNewSubService] = useState("");
  const [editingSubService, setEditingSubService] = useState(null);
  const [subFees, setSubFees] = useState({ SC: 0, ST: 0, OBC: 0, General: 0, Other: 0 });
  ...
  // ===== Socket for realtime services updates =====
  useEffect(() => {
    const socket = getSocket();
    if (!socket) return;

    const handleCreated = (service) => setServices(prev => [...prev, service]);
    const handleUpdated = (service) => setServices(prev => prev.map(s => s._id === service._id ? service : s));
    const handleDeleted = ({ id }) => setServices(prev => prev.filter(s => s._id !== id));
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [appRes, userRes, serviceRes, catRes, noticeRes] = await Promise.all([
          axios.get(`${BASE_URL}/api/applications`, { headers: authHeaders }),
          axios.get(`${BASE_URL}/api/users`, { headers: authHeaders }),
          axios.get(`${BASE_URL}/api/services`, { headers: authHeaders }),
          axios.get(`${BASE_URL}/api/categories`, { headers: authHeaders }),
  ...
  const handleAddOrUpdateSubService = async (serviceId) => {
    ...
    const payload = { name: newSubService.trim(), fees: subFees, platformFee: subPlatformFee };

    try {
      let res;
      if (editingSubService) {
        res = await axios.put(`${BASE_URL}/api/services/${serviceId}/subservices/${editingSubService._id}`, payload, { headers: authHeaders });
      } else {
        res = await axios.put(`${BASE_URL}/api/services/${serviceId}/subservices`, payload, { headers: authHeaders });
      }
      setServices(prev => prev.map(s => s._id === serviceId ? res.data : s));
      resetSubServiceForm();
    } catch (err) {
  ...
  const handleDeleteSubService = async (serviceId, subId) => {
    if (!window.confirm("Delete this subservice?")) return;
    try {
      const res = await axios.delete(`${BASE_URL}/api/services/${serviceId}/subservices/${subId}`, { headers: authHeaders });
      setServices(prev => prev.map(s => s._id === serviceId ? res.data : s));
    } catch (err) {
      console.error("Delete subservice failed", err);
    }
  ...
  // ===== Certificate upload =====
  const handleCertificateFileSelect = (appId, file) => setCertificateFiles(prev => ({ ...prev, [appId]: file }));
  ...
  const handleCertificateUpload = async (e, appId) => {
    ...
    try {
      ...
      const appRes = await axios.get(`${BASE_URL}/api/applications`, { headers: authHeaders });
      setApplications(Array.isArray(appRes.data) ? appRes.data.reverse() : []);
      setCertificateFiles(prev => { const copy = { ...prev }; delete copy[appId]; return copy; });
    } catch (err) {
      console.error("Certificate upload failed:", err);
      alert("Certificate upload failed");
    }
  ...
  const handleHeroUpload = async (e) => {
    ...
    formData.append("image", heroImage);
    try {
      await axios.post(`${BASE_URL}/api/heroslides`, formData, { headers: { ...authHeaders, "Content-Type": "multipart/form-data" } });
      setHeroTitle(""); setHeroSubtitle(""); setHeroImage(null);
      fetchHeroSlides();
    } catch (err) {
      console.error(err);
  ...
  const handleHeroDelete = async (id) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/api/heroslides/${id}`, { headers: authHeaders });
      setHeroSlides(prev => prev.filter(h => h._id !== id));
    } catch (err) {
      console.error(err);
  ...
  // ===== Notice handlers =====
  const handleAddNotice = async () => {
    if (!newNotice.trim()) return alert("Enter notice title");
    try {
      const res = await axios.post(`${BASE_URL}/api/notices`, { title: newNotice, url: newNoticeUrl }, { headers: authHeaders });
      setNotices(prev => [...prev, res.data]);
      setNewNotice(""); setNewNoticeUrl("");
    } catch (err) {
      console.error("Failed to add notice", err);
    }
  };

  const handleEditNotice = (notice) => { setEditedNotice(notice.title); setEditingNoticeId(notice._id); setNewNoticeUrl(notice.url || ""); };

  const handleSaveEditNotice = async () => {
    if (!editingNoticeId) return;
    try {
      const res = await axios.put(`${BASE_URL}/api/notices/${editingNoticeId}`, { title: editedNotice, url: newNoticeUrl }, { headers: authHeaders });
      setNotices(prev => prev.map(n => n._id === editingNoticeId ? res.data : n));
      setEditingNoticeId(null); setEditedNotice(""); setNewNoticeUrl("");
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-4 border-b border-gray-300 pb-2">
        {["applications","users","services","notices","hero"].map(tab => (
          <button
            key={tab}
          ...
          <div className="flex gap-4">
            {["All","Pending","Approved","Rejected"].map(s => (
              <button key={s} onClick={() => setStatusFilter(s)}
                className={`px-3 py-1 rounded ${statusFilter===s ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"}`}>
                {s} ({countByStatus(s)})
              </button>
            ))}
          </div>
              ...
              <tbody>
                {filteredApplications.map((app,i) => (
                  <tr key={app._id} className="hover:bg-gray-50">
                    ...
                    <td className="p-2 border">{app.service?.name || "N/A"}</td>
                    <td className="p-2 border">{app.status}</td>
                    <td className="p-2 border">
                      <input type="file" onChange={e => handleCertificateFileSelect(app._id, e.target.files[0])} className="border p-1 rounded"/>
                      <button onClick={e => handleCertificateUpload(e, app._id)} className="ml-2 px-2 py-1 bg-blue-500 text-white rounded">Upload</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="space-y-6">
          ...
          <div className="border p-4 rounded bg-gray-50 space-y-2">
            ...
            <div className="flex gap-2 flex-wrap">
              <input placeholder="Service Name" value={newService} onChange={e=>setNewService(e.target.value)}
              ...
              <select value={parentService} onChange={e=>setParentService(e.target.value)}
                className="border p-2 rounded">
                <option value="">Parent Service (optional)</option>
                {services.filter(s => !s.parentService).map(s => <option key={s._id} value={s._id}>{s.name}</option>)}
              </select>
            </div>

            {/* Fees Table */}
            <table className="border w-full text-left mt-2">
              <thead>
                <tr className="bg-gray-100"><th className="p-1 border">Category</th><th className="p-1 border">Fee</th></tr>
              </thead>
              <tbody>
                {Object.keys(fees).map(key => (
                  <tr key={key}>
                    ...
                    <td className="p-1 border">
                      <input type="number" value={fees[key]} onChange={e => setFees({...fees,[key]:Number(e.target.value)})} className="border p-1 rounded w-24"/>
                    </td>
                  </tr>
                ))}
                <tr>
                  <td className="p-1 border font-semibold">Platform Fee</td>
                  <td className="p-1 border">
                    <input type="number" value={platformFee} onChange={e=>setPlatformFee(Number(e.target.value))} className="border p-1 rounded w-24"/>
                  </td>
                </tr>
              </tbody>
            </table>

            <button onClick={handleAddOrUpdateService} className="px-3 py-1 bg-green-500 text-white rounded mt-2">
              {editingService ? "Update Service" : "Add Service"}
            </button>
            {editingService && <button onClick={resetServiceForm} className="px-3 py-1 bg-gray-400 text-white rounded mt-2 ml-2">Cancel</button>}
          </div>

          {/* Services List */}
          <div className="space-y-4">
            {services.filter(s => !s.parentService).map(service => (
              <div key={service._id} className="border rounded p-3 bg-white shadow-sm">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleEditClick(service)} className="px-2 py-1 bg-blue-500 text-white rounded">Edit</button>
                    <button onClick={() => handleDeleteService(service._id)} className="px-2 py-1 bg-red-500 text-white rounded">Delete</button>
                  </div>
                </div>
                {/* Subservices */}
                  ...
                  <div className="mt-2 ml-4 space-y-1">
                    {service.subServices.map(sub => (
                      <div key={sub._id} className="flex justify-between items-center">
                        <span>- {sub.name}</span>
                        <div className="flex gap-2">
                          <button onClick={() => handleEditSubClick(sub)} className="px-1 py-0.5 bg-blue-500 text-white rounded text-xs">Edit</button>
                          <button onClick={() => handleDeleteSubService(service._id, sub._id)} className="px-1 py-0.5 bg-red-500 text-white rounded text-xs">Delete</button>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      {selectedTab === "notices" && (
        <div className="space-y-4">
          <div className="flex gap-2">
            <input placeholder="Notice Title" value={newNotice} onChange={e=>setNewNotice(e.target.value)} className="border p-2 rounded flex-1"/>
            <input placeholder="Notice URL" value={newNoticeUrl} onChange={e=>setNewNoticeUrl(e.target.value)} className="border p-2 rounded flex-1"/>
            <button onClick={handleAddNotice} className="px-3 py-1 bg-green-500 text-white rounded">Add Notice</button>
          </div>

          <div className="space-y-2">
            {notices.map(n => (
              <div key={n._id} className="border p-2 rounded flex justify-between items-center bg-white shadow-sm">
                {editingNoticeId === n._id ? (
                  <div className="flex gap-2 flex-1">
                    <input value={editedNotice} onChange={e=>setEditedNotice(e.target.value)} className="border p-1 rounded flex-1"/>
                    <input value={newNoticeUrl} onChange={e=>setNewNoticeUrl(e.target.value)} className="border p-1 rounded flex-1"/>
                  </div>
                ) : (
                  <div className="flex-1">
                    <p className="font-semibold">{n.title}</p>
                    {n.url && <p className="text-xs text-gray-500">{n.url}</p>}
                  </div>
                )}
                <div className="flex gap-2">
                  {editingNoticeId === n._id ? (
                    <button onClick={handleSaveEditNotice} className="px-2 py-1 bg-green-500 text-white rounded text-xs">Save</button>
                  ) : (
                    <button onClick={() => handleEditNotice(n)} className="px-2 py-1 bg-blue-500 text-white rounded text-xs">Edit</button>
                  )}
                  <button onClick={() => handleDeleteNotice(n._id)} className="px-2 py-1 bg-red-500 text-white rounded text-xs">Delete</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {selectedTab === "hero" && (
        <div className="space-y-4">
          <form className="flex gap-2 flex-wrap border p-4 rounded bg-gray-50" onSubmit={handleHeroUpload}>
            <input placeholder="Title" value={heroTitle} onChange={e=>setHeroTitle(e.target.value)} className="border p-2 rounded flex-1"/>
            <input placeholder="Subtitle" value={heroSubtitle} onChange={e=>setHeroSubtitle(e.target.value)} className="border p-2 rounded flex-1"/>
            <input type="file" onChange={e=>setHeroImage(e.target.files[0])} className="border p-2 rounded"/>
            <button type="submit" className="px-3 py-1 bg-green-500 text-white rounded">Upload</button>
          </form>
          <div className="space-y-2">
            {heroSlides.map(h => (
              <div key={h._id} className="border p-2 rounded flex justify-between items-center bg-white shadow-sm">
                <div>
                  <p className="font-semibold">{h.title}</p>
                  <p className="text-xs text-gray-500">{h.subtitle}</p>
                </div>
                <button onClick={() => handleHeroDelete(h._id)} className="px-2 py-1 bg-red-500 text-white rounded text-xs">Delete</button>
              </div>
            ))}
          </div>
        </div>
